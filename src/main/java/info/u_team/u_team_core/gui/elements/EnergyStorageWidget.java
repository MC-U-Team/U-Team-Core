...
public class EnergyStorageWidget extends AbstractWidget implements PerspectiveRenderable {
	...
	public void renderBackground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		...
		for (int yComponent = 1; yComponent < height - 1; yComponent += 2) {
			RenderUtil.addTexturedQuad(bufferBuilder, poseStack, x + 1, x + 1 + 12, getY() + yComponent, getY() + yComponent + 2, 0, 12 / 16f, 0, 2 / 16f, getBlitOffset()); // Background
		}
		
		for (int yComponent = 1 + storageOffset; yComponent < height - 1; yComponent++) {
			if (yComponent % 2 == 0) {
				RenderUtil.addTexturedQuad(bufferBuilder, poseStack, x + 1, x + 1 + 12, getY() + yComponent, getY() + yComponent + 1, 0, 12 / 16f, 3 / 16f, 4 / 16f, getBlitOffset()); // Background
			} else {
				RenderUtil.addTexturedQuad(bufferBuilder, poseStack, x + 1, x + 1 + 12, getY() + yComponent, getY() + yComponent + 1, 0, 12 / 16f, 2 / 16f, 3 / 16f, getBlitOffset()); // Background
			}
		}
	...
	public void renderForeground(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		RenderUtil.drawContainerBorder(poseStack, x, getY(), width, height, getBlitOffset(), RGBA.WHITE);
	}
