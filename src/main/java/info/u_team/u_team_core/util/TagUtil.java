...
public class TagUtil {
	...
	//
	// return helper.wrappers.stream().filter(tag -> tag.getName().equals(location)).map(tag -> (Named<T>)
	// tag).findAny().orElse(helper.bind(location.toString()));
	// }
	//
	...
	// public static <T> IOptionalNamedTag<T> createOptionalTag(ResourceKey<? extends Registry<T>> key, ResourceLocation
	// location) {
	// final StaticTagHelper<T> helper = CastUtil.uncheckedCast(StaticTags.get(key.location()));
	//
	// return helper.wrappers.stream().filter(tag -> tag instanceof IOptionalNamedTag &&
	// tag.getName().equals(location)).map(tag -> (IOptionalNamedTag<T>)
	// tag).findAny().orElse(helper.createOptional(location, null));
	// }
	//
