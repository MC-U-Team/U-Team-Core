...
public class ForgeCreativeModeTabRegister implements CreativeModeTabRegister {
	...
	@Override
	public ResourceEntry<CreativeModeTab> register(String name, BiConsumer<ResourceLocation, Builder> consumer) {
		return register(name, builder -> consumer.accept(new ResourceLocation(modid, name), builder));
	}
	
	@Override
	public ResourceEntry<CreativeModeTab> register(String name, Consumer<Builder> consumer) {
	...
	private void registerTab(CreativeModeTabEvent.Register event) {
		for (final Entry<ForgeCreativeModeTabEntry, Consumer<Builder>> entry : entries.entrySet()) {
