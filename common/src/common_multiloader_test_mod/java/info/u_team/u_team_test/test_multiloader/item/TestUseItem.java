...
public class TestUseItem extends UItem {
	...
	@Override
	public boolean updateItemEntity(ItemStack stack, ItemEntity entity) {
		final Level level = entity.level();
		if (level.isClientSide()) {
			final Vec3 position = entity.position();
			if (level.getRandom().nextInt(10) == 0) {
				for (int i = 0; i < 5; i++) {
					level.addParticle(ParticleTypes.TOTEM_OF_UNDYING, true, position.x(), position.y() + 0.5, position.z(), MathUtil.randomNumberInRange(level.getRandom(), -0.2, 0.2), MathUtil.randomNumberInRange(level.getRandom(), 0.1, 1.5), MathUtil.randomNumberInRange(level.getRandom(), -0.2, 0.2));
				}
			}
		}
		return false;
