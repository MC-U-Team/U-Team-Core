...
public class NeoForgeFluidContainerMenuScreenDelegator<T extends AbstractContainerMenu> implements FluidContainerScreenDelegator {
	
	private final FluidContainerMenuScreen<T> screen;
	
	NeoForgeFluidContainerMenuScreenDelegator(FluidContainerMenuScreen<T> screen) {
		this.screen = screen;
	}
	...
	public void renderLabels(GuiGraphics guiGrapics, int mouseX, int mouseY) {
		if (screen.getMenu() instanceof final FluidContainerMenu fluidMenu) {
			...
			for (int slot = 0; slot < ((NeoForgeFluidContainerMenuDelegator) fluidMenu.getDelegator()).fluidSlots.size(); slot++) {
				final FluidSlot fluidSlot = ((NeoForgeFluidContainerMenuDelegator) fluidMenu.getDelegator()).fluidSlots.get(slot);
	...
	protected FluidSlot findFluidSlot(double mouseX, double mouseY) {
		if (screen.getMenu() instanceof final FluidContainerMenu fluidMenu) {
			for (final FluidSlot fluidSlot : ((NeoForgeFluidContainerMenuDelegator) fluidMenu.getDelegator()).fluidSlots) {
	...
	public List<Component> getTooltipFromFluid(FluidSlot fluidSlot) {
		...
		if (screen.getMinecraft().options.advancedItemTooltips) {
			list.add(Component.literal(RegistryUtil.getBuiltInRegistry(Registries.FLUID).getKey(stack.getFluid()).toString()).withStyle(ChatFormatting.DARK_GRAY));
		}
	...
	public static class Factory implements FluidContainerScreenDelegator.Factory {
		...
		public <T extends AbstractContainerMenu> FluidContainerScreenDelegator create(FluidContainerMenuScreen<T> menu) {
			return new NeoForgeFluidContainerMenuScreenDelegator<>(menu);
		}
	}
}
