...
public class UNeoForgeExtendedTier implements ExtendedTier {
	...
	public UNeoForgeExtendedTier(float[] attackDamage, float[] attackSpeed, TagKey<Block> tag, int uses, float speed, float attackDamageBonus, int enchantmentValue, Supplier<? extends Ingredient> repairIngredient) {
		this(new UExtendedTier(attackDamage, attackSpeed, 0, uses, speed, attackDamageBonus, enchantmentValue, repairIngredient), tag);
	}
	
	public UNeoForgeExtendedTier(ExtendedTier tier, TagKey<Block> tag) {
		this.tier = tier;
		this.tag = tag;
	}
